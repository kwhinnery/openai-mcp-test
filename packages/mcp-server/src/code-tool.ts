...
import { ClientOptions } from 'openai-mcp-test';

const prompt = `Runs JavaScript code to interact with the OpenAI API.
...
async function run(client) {
  const chatCompletion = await client.chat.completions.create({ messages: [{ role: 'user', content: 'Say this is a test' }], model: 'gpt-4o' });
}
\`\`\`
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    OPENAI_API_KEY: requireValue(
      readEnv('OPENAI_API_KEY') ?? client.apiKey,
      'set OPENAI_API_KEY environment variable or provide apiKey client option',
    ),
    OPENAI_ORG_ID: readEnv('OPENAI_ORG_ID') ?? client.organization ?? undefined,
    OPENAI_PROJECT_ID: readEnv('OPENAI_PROJECT_ID') ?? client.project ?? undefined,
    OPENAI_WEBHOOK_SECRET: readEnv('OPENAI_WEBHOOK_SECRET') ?? client.webhookSecret ?? undefined,
    OPENAI_BASE_URL: readEnv('OPENAI_BASE_URL') ?? client.baseURL ?? undefined,
  };
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = 'openai-mcp-test';
  ...
  try {
    const resp = await new Promise<Response>((resolve, reject) => {
      ...
      const opts = {
        ...
        ...(client.organization != null ? { organization: client.organization } : undefined),
