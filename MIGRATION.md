...
To use it, upgrade the `openai-mcp-test` package, then run `./node_modules/.bin/openai-mcp-test migrate ./your/src/folders` to update your code.
...
The `httpAgent` client option has been removed in favor of a [platform-specific `fetchOptions` property](https://github.com/stainless-sdks/openai-mcp-test-typescript#fetch-options).
This change was made as `httpAgent` relied on `node:http` agents which are not supported by any runtime's builtin fetch implementation.

If you were using `httpAgent` for proxy support, check out the [new proxy documentation](https://github.com/stainless-sdks/openai-mcp-test-typescript#configuring-proxies).
...
#### Refactor of `openai-mcp-test/core`, `error`, `pagination`, `resource`, `streaming` and `uploads`

Much of the `openai-mcp-test/core` file was intended to be internal-only but it was publicly accessible, as such it has been refactored and split up into internal and public files, with public-facing code moved to a new `core` folder and internal code moving to the private `internal` folder.
...
```typescript
// Before
import 'openai-mcp-test/error';
import 'openai-mcp-test/pagination';
import 'openai-mcp-test/resource';
import 'openai-mcp-test/streaming';
import 'openai-mcp-test/uploads';

// After
import 'openai-mcp-test/core/error';
import 'openai-mcp-test/core/pagination';
import 'openai-mcp-test/core/resource';
import 'openai-mcp-test/core/streaming';
import 'openai-mcp-test/core/uploads';
```

If you were relying on anything that was only exported from `openai-mcp-test/core` and is also not accessible anywhere else, please open an issue and we'll consider adding it to the public API.
...
```typescript
// Before
const { Completions } = require('openai-mcp-test');

// After
const { OpenAI } = require('openai-mcp-test');
OpenAI.Completions; // or import directly from openai-mcp-test/resources/completions
```

#### Cleaned up `uploads` exports

As part of the `core` refactor, `openai-mcp-test/uploads` was moved to `openai-mcp-test/core/uploads`
...
```typescript
import { type Uploadable, toFile } from 'openai-mcp-test/core/uploads';
```
...
```typescript
// Before
import { APIClient } from 'openai-mcp-test/core';
...
import 'openai-mcp-test/shims/web';
import OpenAI from 'openai-mcp-test';
```

The `openai-mcp-test/shims` imports have been removed. Your global types must now be [correctly configured](#minimum-types-requirements).
...
### `openai-mcp-test/src` directory removed

Previously IDEs may have auto-completed imports from the `openai-mcp-test/src` directory, however this
directory was only included for an improved go-to-definition experience and should not have been used at runtime.

If you have any `openai-mcp-test/src/*` imports, you will need to replace them with `openai-mcp-test/*`.

```ts
// Before
import OpenAI from 'openai-mcp-test/src';
